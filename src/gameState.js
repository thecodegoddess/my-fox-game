import {
  DAY_LENGTH, GAME_STATE, ICON_NAMES, NIGHT_LENGTH, RAIN_CHANCE, SCENE_STATES, WEATHER_STATES,
} from './constants';
...
const gameState = {
  ...
  debugTick(count) {
    ...
    gameDebugger.updateTicker(count);
  },
  debugMsg(msg) {
    if (!this.debug) {
      return;
  ...
  die() {
    ...
    writeModal(
      `Your fox has died!  <br /> Press middle button to start over`,
    );
  ...
  handleUserInteraction(icon, withDebug=false) {
